    ...
    public class AccountController : Controller
    {
        ...
        public JsonResult ForgotPassword(string email)
        {
            if (string.IsNullOrEmpty(email))
                return ResponseHelper.JsonError("Email is required");


            return ResponseHelper.JsonSuccess("Password reset link has been sent to your email.");
        }
        ...
        [HttpPost]
        public JsonResult ResetPassword(string token, string newPassword, string confirmPassword)
        {
            if (string.IsNullOrEmpty(token) || string.IsNullOrEmpty(newPassword))
                return ResponseHelper.JsonError("Invalid request");

            if (newPassword != confirmPassword)
                return ResponseHelper.JsonError("Passwords do not match");

            return ResponseHelper.JsonSuccess("Your password has been reset successfully.");
        }
    }
}